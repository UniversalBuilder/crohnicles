...
# Configuration
DB_PATH = os.path.expanduser('~/Documents/crohnicles.db')  # Windows default path
...
class CrohniclesTrainer:
    ...
    def extract_training_data(self, symptom_type='pain'):
        ...
        FROM events m
        LEFT JOIN (
            SELECT e.id, e.timestamp, e.severity
            FROM events e
            WHERE e.type = 'symptom'
            AND e.tags LIKE '%{symptom_type}%'
            AND e.timestamp >= datetime('now', '-{LOOKBACK_DAYS} days')
        ) s ON (
            julianday(s.timestamp) - julianday(m.timestamp)
        ) * 24 BETWEEN 4 AND {TIME_WINDOW_HOURS}
        WHERE m.type = 'meal'
        AND m.timestamp >= datetime('now', '-{LOOKBACK_DAYS} days')
        ORDER BY m.timestamp DESC
        """
        
        df = pd.read_sql_query(query, self.conn)
        print(f"[Training] Found {len(df)} meal events")
        print(f"[Training] Positive samples (with {symptom_type}): {df['has_symptom'].sum()}")
    ...
    def _extract_features(self, row):
        ...
        tag_features = ['feculent', 'proteine', 'legume', 'produit_laitier', 'fruit',
                        'epices', 'gras', 'sucre', 'fermente', 'gluten', 'alcool']
        for tag in tag_features:
            features[f'tag_{tag}'] = 1.0 if tag.lower() in [t.lower() for t in tags] else 0.0
        ...
        features['is_processed'] = 1.0 if any('industriel' in t.lower() for t in tags) else 0.0
        ...
        features['is_late_night'] = 1.0 if meal_time.hour >= 22 or meal_time.hour < 6 else 0.0
        ...
        # Weather/context (10 features)
        features['temperature_celsius'] = context.get('temperature', 20.0)
        features['pressure_hpa'] = context.get('barometricPressure', 1013.0)
        features['pressure_change_6h'] = context.get('pressureChange6h', 0.0)
        features['humidity'] = context.get('humidity', 50.0)
        features['is_high_humidity'] = 1.0 if context.get('humidity', 50) > 70 else 0.0
        features['is_pressure_dropping'] = 1.0 if context.get('pressureChange6h', 0) < -3 else 0.0
    ...
    def train_model(self, symptom_type):
        ...
        print(classification_report(y_test, y_pred, target_names=['No symptom', 'Symptom']))
    ...
    def export_to_tflite(self, symptom_type):
        ...
        print(f"[Export] Note: For production, integrate with tflite_flutter using TF Decision Forests")
        ...
        def recurse(node_id):
            if tree.feature[node_id] == -2:  # Leaf node
                return {
                    ...
                    'probability': float(tree.value[node_id][0][1] / tree.value[node_id][0].sum())
                }
            else:
                return {
                    'is_leaf': False,
    ...
    def save_training_history(self):
        ...
        for symptom_type, data in self.models.items():
            cursor.execute("""
                INSERT INTO training_history (
                    model_name, trained_at, accuracy, precision, recall, f1_score,
                    feature_importances, training_samples
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                f'{symptom_type}_predictor',
                datetime.now().isoformat(),
                data['accuracy'],
                data['precision'],
                data['recall'],
                data['f1'],
                json.dumps(data['feature_importance'][:10]),  # Top 10
                LOOKBACK_DAYS
            ))
        
        self.conn.commit()
        print(f"[Database] Training history saved for {len(self.models)} models")
...
def main():
    ...
    try:
        ...
        symptom_types = ['pain', 'diarrhea', 'bloating']
        
        for symptom_type in symptom_types:
            try:
                trainer.train_model(symptom_type)
